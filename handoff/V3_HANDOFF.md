...
| **0.18.0** | **Toolchain, ID map, connections — manifest v5 (`toolchain`, identifier `values` per environment and `kind`, `connections`), Tools & versions and Connections in Readiness, read-only sign-in checks, `.vscode/extensions.json` comparison, AI pack fields, the 11 data-goblin plugins** | **this pass, branch `claude/v018-toolchain`** |
...
1. ~~Review and merge 0.13.0 / 0.13.1~~ — merged (PR #18, #19); ~~0.14.0 environment readiness~~ — merged
   ...
   views~~ — merged (PR #25); **0.18.0 toolchain, ID map, connections — this pass**.
2. **Order of the next passes (Julian, [v3/09](v3/09_AI_MODES_WORK_ORDERS_GIT.md) §13.1):** 0.19.0 the
   Git module (AI-1, its own session), then AI-2 work orders, then the **toolkit catalogue**
...
3. **Acceptance with Julian** — testlab project 8 (tools, ID map and connections with the real
   ...
   testlab project 7 (windows and work views, offline, 20–25 minutes), and the FOIL consumer on the
