...
| **0.16.0** | **V3 pass 4: work and DevOps — board (kanban), Azure DevOps / GitHub / GitLab links and CI profiles, Mongoku frozen** | **this pass, branch `claude/v016-work-devops`, on 0.15.1** |
| 0.17.0 | Windows and work views: company workspace file, saved layouts, status-bar switcher, Power Ops launcher | after — a parallel session works on it |
...
1. ~~Review and merge 0.13.0 / 0.13.1~~ — merged (PR #18, #19); ~~0.14.0 environment readiness~~ — merged
   (PR #21); ~~0.15.0 architecture options, project sheet~~ — merged (PR #22); ~~0.16.0 work and
   DevOps~~ — this pass.
2. **Acceptance with Julian** — testlab project 4 (the multi-repository AI loop, offline), testlab
   project 5 (options, sheet, AI files), testlab project 6 (board, Git hosts, CI, Mongoku frozen), and
   the FOIL consumer on the real repositories (its options and sheet are merged in the private
   coordination repository, PR #3): [v3/04_NEXT_PASSES.md](v3/04_NEXT_PASSES.md).
3. **0.17.0 windows and work views** (in progress in a parallel session): [v3/06_VISION_AND_READINESS.md](v3/06_VISION_AND_READINESS.md) section 6.
