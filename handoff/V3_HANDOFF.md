...
| **0.19.0** | **Pass AI-1: the Git module — Git view with Needs you, worktrees, PRs with CI, recent merges, Fetch all, other repositories in `D:\PROJ`, Workbench Git card** | **this pass, branch `claude/ai1-git-module`, on 0.18.0** |
...
1. ~~Review and merge 0.13.0 / 0.13.1~~ — merged (PR #18, #19); ~~0.14.0 environment readiness~~ — merged
   ...
   **0.19.0 the Git module (AI-1) — this pass**.
2. **Order of the next passes (Julian, [v3/09](v3/09_AI_MODES_WORK_ORDERS_GIT.md) §13.1):** ~~0.19.0 the
   Git module (AI-1)~~ — this pass; next AI-2 work orders (desktop apps first, project types
   dev / work / perso, the committed work log), then the **toolkit catalogue**
...
3. **Acceptance with Julian** — testlab project 8 (tools, ID map and connections with the real
   ...
   offline with a stub gh, about 15 minutes, `D:\PROJ\datapass-testlab\7a-git`), and the FOIL consumer on the
