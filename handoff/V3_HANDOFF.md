...
| **0.14.0** | **V3 pass 2: environment readiness + companion ergonomics — manifest v4 (`localEnv`, `identifiers`), Local environment / Readiness sections in the Project view, env-file and Power Ops commands, names-and-states-only in the snapshot and AI context** | branch `claude/youthful-sanderson-3d31af`, PR pending |
...
Architecture detail: [v3/02_ARCHITECTURE.md](v3/02_ARCHITECTURE.md). How an AI prepares a project:
...
1. ~~Review and merge 0.13.0~~ — merged (PR #18); ~~0.13.1~~ — merged (PR #19), fixed the `$schema`
   warning seen on first use. 0.14.0 (this handoff) adds environment readiness; review and merge it.
2. **Acceptance with Julian** — testlab project 4 (the multi-repository AI loop, offline) and the FOIL
   consumer on the real repositories: [v3/04_NEXT_PASSES.md](v3/04_NEXT_PASSES.md).
3. **Account qualification** (V1 gate 16, unchanged): Databricks validate, Fabric browse, Azure plan,
   VM over SSH — now also Azure Functions run-local/deploy and ADF Studio on a small STUDY slice.
4. Then 1.0.0. After 1.0: Restricted Mode and Remote-SSH/WSL desktop qualification, richer catalog,
   DiagramCloud and Mongoku as optional modules (last, per Julian's priorities).
