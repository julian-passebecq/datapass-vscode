...
## What changed
- **Stamp** = selected variant (its title and the picks that differ from the current architecture; the picks are the compared key, so scenario B and the same picks chosen by hand match), environment, bridge revision (HEAD of the coordination repository). Pure module `src/core/project/packStamp.ts`; `src/work/packStamps.ts` computes it for the open project.
...
- **Stale packs**: every `ai-context` exchange record now keeps the variant/environment it was copied for (`session.recordExchange`, additive `ExchangeRecord.stamp`). Copy Context for My AI is now recorded too. The AI view's "Last exchanges" shows `⚠ Stale: built for B …; the selected variant is now C …` and refreshes on a variant switch. A newer bridge revision alone does not make a pack stale.
...
## Tests
- Unit (`tests/workOrders.test.ts`, `tests/fileContext.test.ts`): stamp from picks/preview, environment rule, stamp line; stale on variant or environment change, not on bridge move; order.json + order.md + Ajv schema; verdict same / other-variant / not-stamped; old unstamped order parses; malformed stamp refused; AI view stale reason; file-context header.
...
- One-line hooks outside the package's file list: `src/work/session.ts` (stamp on record), `src/core/work/workModel.ts` (optional field), `src/work/fileContextCommands.ts` and `src/work/optionsCommands.ts` (pass the stamp; record the file-context copy), `src/views/agentState.ts` (row label).
