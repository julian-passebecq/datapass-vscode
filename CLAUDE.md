...
V2.2 refines V2.1; it does not restart the extension or replace native provider schemas. Read the retained V2.1 items/artifacts/workflows, Airflow, portability, Git/workspace and AI-security design as the foundation. Current source establishes implemented behavior; a handoff version is not an extension release. Current source: v0.15.1 (AI exchange view in the secondary side bar, shown instead of Chat when a DataPass project opens) on 0.15.0 (V3 pass 3: architecture options and scenarios, project sheet, diagram views, JSON exchange with an AI with backups), on top of 0.14.0 (V3 pass 2: environment readiness, manifest v4, from a parallel session, PR #21). The global vision, what VS Code allows for companies/workspaces/tabs, readiness per step and the next passes (0.16 work and DevOps, 0.17 windows and work views) are in [handoff/v3/06_VISION_AND_READINESS.md](handoff/v3/06_VISION_AND_READINESS.md). Handoffs pasted from other chats can be stale: check `git log`, open/merged PRs and `git worktree list` (other checkouts of this repository may exist beside it) before trusting one.
