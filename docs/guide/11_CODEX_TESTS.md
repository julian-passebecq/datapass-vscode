...
- `purpose: "app"`: tests DataPass itself across several clients: `workspaces[]` (1 to 10), each
  `{client, bridge, repositories[]}`. The journeys are `kind: "app"`.
...
- Folders and paths are relative (no `..`, no drive, no leading `/`), remotes are `https://` with no
  credentials, a folder is declared with only one remote. The VSIX is a local path: there is no
...
evidence, confidence high/medium/low}`), `clientFeedback`, `coverage {listed, reached}`.
...
4. Writes one `<client id>.code-workspace` per client (bridge folder first) and `run.json`
   (`datapass.qa-run` v1: run id, VSIX sha256, VS Code version, each clone's commit, the launch commands).
5. Prints the command that opens each client in the isolated VS Code.
