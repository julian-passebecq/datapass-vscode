...
export interface FileContextInput {
  ...
  /** 0.25 (V-A): the active variant line (title and coding state), when the project has options. */
...
export function buildFileContext(input: FileContextInput, options: FileContextOptions = {}): FileContextPack {
  ...
  if (input.project && input.activeVariant) lines.push(input.activeVariant);
  if (input.project && input.stamp) lines.push(stampLine(input.stamp));
