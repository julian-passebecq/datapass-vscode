...
export const RELATIONS = ["contains", "uses", "produces", "consumes", "runsOn", "deployedFrom", "observedBy", "derivedFrom", "supersedes", "dependsOn", "invokes"] as const;

const SHORT: Schema = { type: "string", minLength: 1, maxLength: 200 };
const REL_PATH: Schema = { type: "string", minLength: 1, maxLength: 400, pattern: "^(?![/\\\\~])(?!.*(^|[/\\\\])\\.\\.([/\\\\]|$)).+$" };
...
export interface ProjectGraph {
  format: "datapass.graph"; version: "0.1-draft";
...
function isStringMap(v: unknown): boolean {
  return !!v && typeof v === "object" && !Array.isArray(v) && Object.entries(v).every(([k, x]) => /^[a-z][a-z0-9_.-]{0,79}$/.test(k) && typeof x === "string" && x.length <= 200);
}
...
export function emptyGraph(): ProjectGraph {
  return { format: "datapass.graph", version: "0.1-draft", roles: {}, items: [], outputs: [], relations: [] };
}
