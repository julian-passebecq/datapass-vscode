...
async function upgradeManifest(session: WorkSession): Promise<void> {
  ...
  if (!(await confirmModal(`Upgrade .datapass/project.json to schemaVersion ${LATEST_MANIFEST_VERSION}?`, `A copy of the current file goes to ${backup}. Every field is kept.${ctx.manifest.schemaVersion < 3 ? " v3 adds environments, project docs, planned repositories and a default repository per sub-project." : ""}${ctx.manifest.schemaVersion < 4 ? " v4 adds localEnv (the env files and variable names the project needs, never values) and identifiers (explicitly non-secret ids)." : ""} v5 adds toolchain (the tools and version ranges the project needs), identifiers with one value per environment (the ID map) and connections (sign-ins and bindings). Older DataPass versions will refuse a v${LATEST_MANIFEST_VERSION} file.`, "Upgrade"))) return;
