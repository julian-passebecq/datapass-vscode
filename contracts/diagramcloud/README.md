...
## V1 implementation notes (additive, 2026-09-24)

These clarify how DataPass implements this contract. Neither schema changes.

### Revisions

- `base.projectManifestRevision` is `sha256:<hex>` of the exact bytes of `.datapass/project.json`.
- `base.diagramCloudRevision` is the integer `revision` of `.datapass/diagramcloud.json`, or `null` when there is no readable sidecar.
- `base.repositoryCommit` is the workspace `HEAD`, suffixed `+dirty` when the working tree has changes. It is informational and not checked on import.

A plan is rejected when either base revision differs from the files on disk at import time. The sidecar write is also journaled against the SHA-256 of the exact bytes that were reviewed, so a DiagramCloud save between review and write is never overwritten.

### Operations DataPass V1 applies

Only these operations can be approved and written, and only to `.datapass/diagramcloud.json`. The revision increases by one per applied plan:

| action | target | entityId | payload |
| --- | --- | --- | --- |
| `set-project-metadata` | `diagramcloud-document` | – | `{ "title"?: string (1–160), "summary"?: string (≤ 3000) }`; no other keys |
| `link-node-workspace` | `diagramcloud-document` | existing node id | `{ "workspaceId": existing experience workspace id }` |

All other actions, and every `project-manifest` operation, are schema-valid but review-only in V1. DataPass lists them with a reason and does not write them. Add payload shapes here, with fixtures from both products, before making another action applicable.

DataPass writes the sidecar with DiagramCloud's own serialization (two-space JSON, trailing newline, DiagramCloud key order), so a following DiagramCloud save produces no Git diff. The DataPass test fixture `tests/fixtures/diagramcloud/total.after-plan.sidecar.json` is generated by DiagramCloud and pins this.

### Context for an AI chat

`datapass.ai-context` stays a closed envelope. DataPass's "Copy with instructions" wraps it in a short prompt that restates the base revisions and the applicable operations. When a sidecar exists, the prompt also lists the IDs and titles (no content) of its nodes and experience workspaces, so an AI can reference them. "Copy JSON only" copies the envelope alone.
