...
test("inventory: Databricks notebooks, Airflow DAGs and ADF pipelines are read statically, never run", () => {
  ...
  // F08: an Azure Data Factory pipeline belongs to the Azure module, not to Fabric.
  assert.equal(adf.module, "azure");
  // F09: an Azure Functions app is recognised from its v2 host.json; Terraform and Bicep roots too.
  const fn = classifyAsset("functions/extract/host.json", JSON.stringify({ version: "2.0", extensionBundle: { id: "Microsoft.Azure.Functions.ExtensionBundle" } }))!;
  assert.deepEqual([fn.kind, fn.name, fn.module], ["azure-functions", "extract", "azure"]);
  assert.equal(classifyAsset("other/host.json", JSON.stringify({ version: "1.0" })), undefined);
  assert.equal(classifyAsset("infra/main.tf")?.kind, "iac");
  assert.equal(classifyAsset("infra/storage.bicep")?.module, "infrastructure");
...
test("repositories: local Git state from porcelain v2, and plain wording for each state", () => {
  ...
  assert.deepEqual(clean, { changes: 0, trackedChanges: 0, head: "0123456789abcdef0123456789abcdef01234567", branch: "main", upstream: "origin/main", ahead: 0, behind: 0 });
  ...
  // A modified tracked file blocks a fast-forward; an untracked file does not.
  assert.deepEqual([dirty.changes, dirty.trackedChanges], [2, 1]);
