...
test("card pack: the card, its components and files, the scrubbed error, the rules to move the card", () => {
  ...
  // V1-STAB (P1 carry-over): the card and preparation packs carry the stamp, right under the header.
  const stamp = { variant: { key: "orchestration=blob-function", title: "B — Blob event", picks: ["orchestration=blob-function"] }, environment: "dev", bridge: "a".repeat(40) };
  for (const t of [
    buildCardPack({ board: boardA(), card, map: m, question: "decide", dataPassVersion: "1", generatedAt: "t", stamp }).text,
    buildPreparationPack({ map: m, componentId: "extract", question: "explain", dataPassVersion: "1", generatedAt: "t", stamp }).text
  ]) assert.match(t, /^# [^\n]+\nGenerated by DataPass 1 on t\.[^\n]*\nStamp: built for the selected variant \*\*B — Blob event\*\* \(orchestration=blob-function\) · environment dev · bridge revision aaaaaaaaaaaa\./);
  assert.ok(!prep.text.includes("Stamp:"), "no stamp given, no line");
});
